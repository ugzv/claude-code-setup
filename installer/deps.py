...
from .platform import IS_MACOS, IS_WINDOWS, IS_WSL


def _check_and_brew_install(
    package: str, dry_run: bool, required: bool = False
) -> None:
...
def install_macos_deps(dry_run: bool = False) -> None:
    """Install macOS dependencies: terminal-notifier (notifications)."""
    if not IS_MACOS:
        return
...
def install_wsl_deps(dry_run: bool = False) -> None:
    """No required external deps on WSL; native Linux gets optional desktop tool hints."""
