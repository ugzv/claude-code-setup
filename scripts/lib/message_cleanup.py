...
def detect_action_emoji(text: str) -> str:
    ...
    if any(
        word in text_lower
        for word in ["fix", "bug", "error", "issue", "resolve", "correct"]
    ):
        return "🐛"
    if any(
        word in text_lower
        for word in ["edit", "update", "modify", "change", "improve", "refactor"]
    ):
        return "✏️"
    if any(
        word in text_lower
        for word in ["search", "find", "look", "explore", "check", "review"]
    ):
        return "🔍"
    if any(
        word in text_lower for word in ["create", "add", "write", "implement", "build"]
    ):
        return "✨"
...
def should_skip_line(first_line: str) -> bool:
    ...
    return first_word in SKIP_PHRASES or any(
        ...
        or first_line_lower.startswith(phrase + "!")
        for phrase in SKIP_PHRASES
    )


def find_content_line(
    text_lines: list[str], start_index: int = 0
) -> tuple[str | None, int]:
